...
if mode == "网页URL爬取分析":
    ...
    manual_text = st.text_area(
        "请输入待分析文本",
        height=200,
        placeholder="示例：今天天气很好，明天天气也不错，后天适合出门散步"
    )
    ...
    if st.button("📊 开始文本分析", use_container_width=True):
        ...
        if top_keywords:
            ...
            with col4:
                ...
                # 用matplotlib绘制饼图
                fig, ax = plt.subplots()
                ax.pie(counts, labels=words, autopct="%1.1f%%", colors=["#1f77b4", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd", "#8c564b"])
                ax.axis("equal")  # 保证饼图是正圆形
                st.pyplot(fig)  # 嵌入Streamlit
        else:
            st.info("📌 未提取到有效关键词")
else:
